...
Redditor [minrice2099](http://www.reddit.com/r/webdev/comments/2023ex/windowclusterfuck_a_tale_of_global_variables/cfz7qb9) [<i class="fa fa-github"></i>](Github) came up with a clever way to check if a variable comes from a named element. Checking <code class="language-javascript">x in Object.keys( window )</code> will return <code>False</code> if <code>x</code> was created by the browser from a named element, but <code>True</code> otherwise. From here, we can check existence using:
...
It's unfortunate to have to think about this, but it's for the better. Works in all modern browsers, but use at your own risk; who knows how this will be implemented in the future.
